...
import { users, customers, invoices, revenue, latestInvoces, filteredInvoices } from './placeholder-data'
...
export async function fetchCardData() {
  ...
  return {
    numberOfCustomers: 8,
    numberOfInvoices: 15,
    totalPaidInvoices: formatCurrency(110636),
    totalPendingInvoices: formatCurrency(133911),
  };
}
...
export async function fetchInvoicesPages(query: string) {
  if (query) {
    return Math.ceil(filteredInvoices
      .filter(q => q.name.includes(query) || q.email.includes(query) || q.status.includes(query))
